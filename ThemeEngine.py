...
def get_background_ids(list_of_paths):
    return list(map(lambda x: os.path.splitext(os.path.split(x)[1])[0], list_of_paths))
...
# world to screen transform
    ...
    def get_default_theme(self):
        ...
        default_themes = list(map(lambda x: os.path.basename(x), self.config_db["themes"]))
    ...
    def getavailable_themes(self):
        result = sorted(self.getavailable_themes_dict().keys())
        return result
